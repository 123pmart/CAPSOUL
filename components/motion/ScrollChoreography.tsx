...
function useStaggeredCardMotion(
  ...
  const y = useTransform(progress, [start, end], isMobile ? ["24px", "0px"] : ["70px", "0px"]);
  const opacity = useTransform(progress, [start, end], isMobile ? [0.68, 1] : [0, 1]);
...
export function ScrollChoreography({
  ...
  const panelScale = useTransform(scrollYProgress, [0, 0.45], isMobile ? [0.985, 1] : [0.92, 1]);
